...
import { getAccountData, addTransactionAction, updateTransactionAction } from '@/actions/app';
...
export default function AddExpensePage({
  ...
  const { tx: txId } = use(searchParams);
    ...
    const fetchData = async () => {
      try {
        ...
        if (res.success && res.account) {
          ...
          if (txId) {
            const tx = (res.transactions || []).find((t: any) => t.id === txId);
            if (!tx) {
              router.push(`/account/${id}`);
              return;
            }
            const txEntries = (res.entries || []).filter((e: any) => (e.transactionId || e.transaction_id) === txId);
  ...
  return (
    <div className="bg-surface text-on-surface min-h-screen flex flex-col antialiased pt-16">
      <Header title={isEditing ? "Editar Gasto" : "Añadir Gasto"} showBack />
