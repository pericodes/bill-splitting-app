...
export default function AccountPage() {
  ...
  const handleDelete = (tx: { id: string; description?: string }) => {
    showConfirm({
      ...
      description: `¿Quieres eliminar «${tx.description || "este gasto"}»? Esta acción no se puede deshacer.`,
      isDestructive: true,
      ...
      onConfirm: async () => {
        ...
        try {
          ...
          if (!res.success) {
            showAlert({ title: "Error", description: res.error || "No se pudo eliminar el gasto." });
            return;
          }
          await refetch();
        } finally {
